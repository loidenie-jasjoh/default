...
ig.Game = ig.Class.extend({
    ...
    setSpawnLocations: function(data) {
        var spawnLayerIndex = 0;
        var collisionLayerIndex = 1;

        var spawnLayerTileIndexArray = data.layer[spawnLayerIndex].data;
        var collisionLayerTileIndexArray = data.layer[collisionLayerIndex].data;

        var tileSize = data.layer[spawnLayerIndex].tilesize;

        var collisionLayerRows = collisionLayerTileIndexArray.length;
        var collisionLayerColumns = collisionLayerTileIndexArray[0].length;

        // loop through tiles to find valid spawn locations
        for (var iR = 1; iR < collisionLayerRows-1; iR++) { // loop through collision layer rows
            for (var iC = 0; iC < collisionLayerColumns; iC++) { // loop through each column (index) in a row
                var currentCollisionTileIndex = collisionLayerTileIndexArray[iR][iC];
                if (currentCollisionTileIndex === 0) { // this is not a collision tile (GOOD)
                    var collisionTileBelowCurrent = collisionLayerTileIndexArray[iR+1][iC];
                    if (collisionTileBelowCurrent === 1) { // tile below IS collision (GOOD)
                        var spawnTileAbove = spawnLayerTileIndexArray[iR-1][iC];
                        if (spawnTileAbove !== 0) { // tile above is NOT empty (GOOD)
                            // this.randomSpawns.push({x:iR-1, y:iC-1}); // add tile to valid spawn set
                            this.randomSpawns.push({x:(iC+1)*tileSize, y:(iR+1)*tileSize}); // add tile to valid spawn set
                        }
                    }
                }
            }
    ...
    getRandomSpawnLocation: function() {
        var numLocations = this.randomSpawns.length;
        // var foo = Math.random();
        var randomLocation = Math.floor(Math.random() * numLocations);
        return this.randomSpawns[randomLocation];
    }
});
